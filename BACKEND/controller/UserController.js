...
const updatePassword = asyncHandler(async (req, res) => {
  const { _id } = req.user;
  console.log('id from req.user: ', _id);
...
const forgotPasswordToken =  asyncHandler( async( req,res) => {
  ...
  try{
      ...
      await user.save();
      const resetURL = `Hi, please follow this link to reset your password. This link is valid till 10 minutes from now. <a href='http://localhost:5051/api/user/reset-password/${token}'>Click Here</>`;
      const data={ 
          to:email,
          text:"Hey User",
